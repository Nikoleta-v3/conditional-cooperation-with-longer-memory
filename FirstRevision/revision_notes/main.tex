...
\begin{figure}[tbhp]
  ...
  We repeat the analysis as in Figure~\ref{fig:InvasionAnalysisReactive2}, but
  this time we consider the case where \(p_{CC} = 1\). We observe that in this
  case, each of the residents \(\mathbf{p}\) and \(\mathbf{p'}\) have the
  same average number until invasion, and the expected payoffs in the case of \(k\)
  \alld{} mutants are the same.
...
\begin{table}
  ...
  \begin{tabular}{c|rlccc|rlccc}
    ...
    \toprule
    \thead{Most common \\ paths} &   & Path &  \thead{Recovery \\ time} & Path probability & \thead{Relative abundance \\ among $5 \times 10^4$ paths} & & Path &   \thead{Recovery \\ time} & Path probability&  \thead{Relative abundance \\ among $5 \times 10^4$ paths}\\
    \midrule
    ...
    15 &  \cellcolor{gray!25} \makecell[r]{CC \\ CD} & \makecell[l]{DCDCDCC \\  CDCDCCC} &  7 &  0.0020 & 0.3\%  &   \cellcolor{gray!25} \makecell[r]{CC \\ CD} & \makecell[l]{        DCCCC \\         CCDCC} &   5 &   0.001 & 0.1\% \\ 
    \bottomrule
    \end{tabular}}
    \caption{We use the Axelrod-Python~\cite{AxelrodPython} package to simulate the
    interactions between two players. First, when both players are using the
    strategy \(\mathbf{p} = (0.99, 0.10, 0.60, 0.30)\), and then when they are using
    the strategy \(\mathbf{p'} = (0.99, 0.60, 0.10, 0.30)\). We assume that the
    players start with both cooperating, followed by the second player defecting,
    resulting in the history \( ((C, C), (C, D)) \). We simulate the interaction and
    record how long it takes for them to return to \( ((C, C), (C, C)) \). We repeat
    this \(5 \times 10^4\) times and record the average time it takes for the
    players to return to \( ((C, C), (C, C)) \), along with the path of actions
    their strategies followed. We call the sequence of actions leading to mutual
    cooperation for two rounds a path. The length of this path is what we refer to
    as recovery time. We present the fifteen most common paths for both strategies.}
\end{table}
