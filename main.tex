...
%We are easily influenced by how cooperative others are, we like to return favors, and we show more effort when others do. 
We are easily influenced by how cooperative others are, and we adapt our behavior accordingly. 
To describe reciprocal cooperation formally, models often presume that individuals only react to their very last interaction. 
Instead, here we allow individuals to react to an opponent's $n$ previous actions, for arbitrary $n$.
We derive an algorithm to characterize all partner strategies -- strategies that sustain full cooperation in a Nash equilibrium.
...
This game considers two individuals, referred to as players. 
The players repeatedly decide whether to cooperate or to defect with one another~(\figurref{fig:conceptual_figure_model}\textbf{A}). 
...
Here, cooperation simply means to pay a cost $c\!>\!0$ for the co-player to derive a benefit $b\!>\!c$. 
...
Strategies of the repeated Prisoner's Dilemma can vary in how complex they are.
...
Similarly, one can distinguish strategies that require more than one round of memory, or strategies that require more than finite memory~\cite{Garcia:FRAI:2018}. 
...
Although one-round memory can explain some of the empirical regularities in human behavior~\cite{engle:ET:2006, dal:AER:2011, camera:GEB:2012, bruttel:TD:2012,Montero-Porras:SciRep:2022}, there is evidence that many people take into account more than the last round~\cite{romero:EER:2018}.
...
However, a systematic analysis of strategies with more than one-round memory has been difficult for two reasons. 
...
Some of the resulting strategies have an intuitive interpretation, such as Tit-for-Two-Tat~\citep{axelrod:AAAS:1981}, but many others are difficult to make sense of. 
...
Probably for these reasons, previous studies were either restricted to simulations for small $n$~\citep{hauert:PRSB:1997,stewart:scientific:2016,Murase:PLoSCompBio:2023a}, or they analyzed the properties of particular higher-memory strategies~\citep{hilbe:PNAS:2017,ueda:RSOP:2021,li:NatureCompSci:2022}. 


To make progress, we focus on a specific subset of memory-$n$ strategies, the {\it reactive}-$n$ strategies. 
A player with such a strategy only takes into account the {\it co-player's} actions during the last $n$ rounds~(\figurref{fig:conceptual_figure_model}\textbf{C,E}). 
These strategies often allow for a straightforward interpretation. 
Moreover, they capture the basic premise of conditional cooperation: one player's behavior is contingent on the other player's previous actions. 
While it has been difficult to explicitly characterize all Nash equilibria among the memory-$n$ strategies, we show that such a characterization is feasible for reactive-$n$ strategies. 
...
Based on this insight, we characterize all partner strategies among the reactive-2 and the reactive-3 strategies. 
These are the strategies that sustain full cooperation in a Nash equilibrium. 
We can derive even stronger results if we restrict attention to so-called {\it counting strategies}.  
Such strategies only react to how often their co-player has cooperated in the last $n$ rounds (irrespective of the exact timing of cooperation). 
...
For every defection of the co-player in memory, the focal player's cooperation rate needs to drop by at least $c/(nb)$.
