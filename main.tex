...
We consider two players infinitely repeated games. In each round, players can
choose to cooperate ($C$) or defect ($D$). If both players cooperate they
receive a payoff $R$ (the reward for mutual cooperation), and if both players
defect then they receive a payoff $P$ (the punishment for mutual defection). If
one player cooperates, the cooperative player receives the sucker's payoff $S$,
and the defecting players the temptation payoff $T$. The payoffs are such that
$T > R > P > S$. Herein. we use a specific parametrization of the prisoner's
dilemma, where cooperation means paying a cost $c$ for the co-player to derive a
benefit $b>c$. As a result, the payoffs because $R=b-c, S=-c, T=b, P=0$. In the
Appendix we show that the main results of this work hold for the general
prisoner's dilemma.
