...
The play between two two-bit reactive strategies can be described by a Markov
...
\begin{algorithm}[H]
  \SetAlgoLined \For{ $i <$ maximum number of points}{
  \uIf{type is reactive}{
    \(\mathbf{p} = \mathbf{\tilde{p}}\) $\leftarrow$ random: $\{\emptyset \}\ \rightarrow R_{[0, 1]}^4$\;
  }
  \Else{
    \(\mathbf{p}\) $\leftarrow$ random: $\{\emptyset \}\ \rightarrow R_{[0, 1]}^{16}$\;
  }
   \(p_1\)
   $\leftarrow$ 1\;
   $L(\mathbf{p}) = \{\mathbf{q} \,|\, \pi(\mathbf{p}, \mathbf{p}) \geq \pi(\mathbf{q}, \mathbf{p}) \text{ for } \mathbf{q} \in \{\mathbf{q_i} \in \binom{\{0, 1\}}{16} \,|\, |\mathbf{q_i}| = 4 \}\}$\;
...
Martinez-Vaquero method allows us to numerically evaluate if pure strategies are Nash given that
